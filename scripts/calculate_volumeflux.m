...
radius1 =6216000;  %lower end of radius 5966000   6096000   6291000   6216
radius2 = 6256000;  %high end of radius 6026000    6136000   6331000   6256
...
%PathStr=['/home/djneuh/Desktop/dislocation_runs/m3-1_dislocation-8cm/data'];
%PathStr=['/home/djneuh/Desktop/dislocation_runs/m' num2str(v) '-1_dislocation-8cm/data'];
%PathStrp=['/media/djneuh/Elements/pulsing_plumes/Thesis models/30-km/movingtop/4e6/4e6-1e6-30km-8cmtop/datap/'];
PathStrp=['/home/djneuh/Desktop/dislocation_runs/tolerance_test/2-2_hightol/data/'];
PathStrf=['/home/djneuh/Desktop/dislocation_runs/tolerance_test/2-2_hightol/data/'];

%create a variable linked to the directory with data files
textFilesf=dir([PathStrf '*.csv']);
textFilesp=dir([PathStrp '*.csv']);
...
for k = 1:1600 %(length(textFiles)-1) or just a number > than that
  ...
  fullname = fullfile(PathStrp, textFilename);
  ...
  fullname = fullfile(PathStrf, textFilename);
  fid=fopen(fullname, 'rt');
...
for i=1:length(test2)
    ...
    Dd(i,5) = test2(i,5);       %%Non-adiabatic temperature normally 15
end
...
Vrin(:,3) = griddata(Dd(:,3), Dd(:,4), Dd(:,5), xq, yq);
Vrin(:,2) = theta;
...
for df=1:length(Vrin(:,1))
    if Vrin(df,2) < ptheta+2.5 && Vrin(df,2) > ptheta-2.5
...
save(['volumeflux_100km'], 'final410')
%save(['4-1_' num2str(v) 'cm_volumeflux100km_new'], 'final410')
%clearvars -except v
