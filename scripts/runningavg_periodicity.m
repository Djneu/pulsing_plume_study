%%This is the final version script for geting the running average of
%%periodicities
% Will look at 80 Myr to 200 Myr every 10 Myr, with 30 Myr windows, given an
% actual time of 65 Myr to 215 Myr.
...
TF = islocalmax(P); %'MinProminence', 5*10^8, 'MinSeparation',minutes(45),'SamplePoints',t  'MaxNumExtrema', 5
PyyP = P(TF);
periodp(:,1) = period(TF);
...
periodicity(yy,2) = yy;
clearvars -except yy final410 periodicity maxyear interval
end
% figure(3)
 %plot(period,P,periodp,PyyP,'r*')
% xlim([0 xy]);

clearvars -except periodicity maxyear
